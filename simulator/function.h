	...
	int func_Num = change_to_UInt(funct,6);
	 ...
	 if(op_Num==0){ 				////////// RType
		...
		*is_Jumping = 0;
        	...
        	*is_AluSrcImme = 0;
	        *is_RegWrite = 1;
			if(func_Num==0x08) *is_RegWrite = 0; //jr don't write regs
		*is_SystemCall =0;
	}
		...
		*is_Jumping = 0;
		*is_Branching = 0;
			if(op_Num==0x04 || op_Num==0x05 || op_Num==0x07) *is_Branching=1; // beq,bne,bgtz branch
		*is_MemRead = 0;
			if(op_Num==0x23) *is_MemRead = 1;
			else if(op_Num == 0x21 || op_Num == 0x25) *is_MemRead = 2;
			else if(op_Num == 0x20 || op_Num == 0x24) *is_MemRead = 3;
		*is_MemToReg = 0;
			if(op_Num==0x23 || op_Num==0x21 || op_Num==0x25 || op_Num==0x20 || op_Num==0x24) *is_MemToReg = 1;
		*ALUOP = 0;
			if(op_Num==0x04 || op_Num==0x05 || op_Num==0x07) *ALUOP = 1;
			else if(op_Num==0x08 || op_Num==0x09 || op_Num==0x0C || op_Num==0x0D || op_Num==0x0E || op_Num==0x0A || op_Num==0x0F) *ALUOP=3;
		*is_MemWrite = 0;
			if(op_Num==0x2B) *is_MemWrite = 1;
			else if(op_Num==0x29) *is_MemWrite = 2;
			else if(op_Num==0x28) *is_MemWrite = 3;
		*is_AluSrcImme = 1;
			if(op_Num==0x04 || op_Num==0x05 || op_Num==0x07) *is_AluSrcImme = 0;
		*is_RegWrite = 1;
			if(op_Num==0x2B  || op_Num==0x29  || op_Num==0x28  || op_Num==0x04  || op_Num==0x05  || op_Num==0x07  ) *is_RegWrite = 0;
		*is_SystemCall = 0;
...
void extend_Imme(int* imme, int* exImme, int* opcode){
	...
	if(op_Num == 0x0C || op_Num == 0x0D || op_Num == 0x0E ){
		needSign = 0;
	}
...
void register_Alu(int* result,int* input1, int* input2 ,int alu_Control,int* nextPC, int* shamt, bool* error_buffer,int is_Branching){
	...
	switch(c){
		case 0: pc_ADD(result,input1,input2,error_buffer);
				break;
		case 1: SUB_funt(result,input1,input2,is_Branching?handling:error_buffer);
				break;
		case 2: AND_funt(result,input1,input2);
				break;
		case 3: OR_funt(result,input1,input2);
				break;
		case 4: XOR_funt(result,input1,input2);
				break;
		case 5: NOR_funt(result,input1,input2);
						break;
		case 6: NAND_funt(result,input1,input2);
						break;
		case 7: SLT_funt(result,input1,input2);
				break;
		case 8: SLL_funt(result,input2,shamt);
							break;
		case 9: SRL_funt(result,input2,shamt);
						break;
		case 10: SRA_funt(result,input2,shamt);
						break;
		case 12: ADDU_funt(result,input1,input2);
					break;
		case 13:LUI_funt(result,input2);
				break;
	}
...
cont:;
	...
	if(op_Num==0&&(fNum==0x00||fNum==0x02||fNum==0x03)){
		need1=0;
	}
	else if(op_Num==0x0F){
		need1=0;
...
void hazard_Controller(){
	...
	if(a||b){
		...
		if(a && (ID->is_Branching|| (change_to_UInt(ID->opcode,6)==0&&change_to_UInt(ID->funct,6)==0x08)  ) ){
			// branch need fix
			if(DM->is_Jal==1&&a==1){
	...
	//printf("stall\n");
	int is_Jr = change_to_UInt(ID->opcode,6)==0 && change_to_UInt(ID->funct,6)==0x08;
